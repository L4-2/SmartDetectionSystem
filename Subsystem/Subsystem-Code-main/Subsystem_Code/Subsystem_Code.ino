...
bool connectToServer(BLEAdvertisedDevice* device) {
  ...
  std::string serviceUUIDString = serviceUUID.toString().c_str();
  ...
  std::string characteristicString = characteristic->toString().c_str();
...
void AdvertisedDeviceCallback::onResult(BLEAdvertisedDevice advertisedDevice) {
  ...
  if (advertisedDevice.getName() == std::string("ESP32").c_str()) {
    Serial.println("Located main ESP32");
    BLEAddress targetAddress = advertisedDevice.getAddress();
    std::string addressString = targetAddress.toString().c_str();
